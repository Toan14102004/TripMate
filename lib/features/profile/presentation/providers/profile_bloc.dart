...
class ProfileCubit extends Cubit<ProfileState> {
  ProfileCubit()
    : super(ProfileData(fullname: '', dob: DateTime.now(), email: ''));
  ...
  Future<void> onChangeProfile({
    ...
    if (state is ProfileData) {
      ...
      emit(currentState.copyWith(fullname: fullname, dob: dob, email: email));
    }
  }
  ...
  Future<void> logout() async {
    ...
    result.fold(
      ...
      (right) {
        ...
        emit(ProfileData(fullname: '', dob: DateTime.now(), email: ''));
      },
    );
  }
}
